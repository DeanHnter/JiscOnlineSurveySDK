...
func (innerchild *InnerContainer) AddOption(text string, screen_to_message bool, routePage *PageContainer, is_other bool, is_not_applicable bool) {
	...
	if innerchild.Class == MultipleChoice || innerchild.Class == MultipleAnswer || innerchild.Class == SelectionList {
		id, _ := CreateID()
		var option Option
		if routePage == nil {
...
func (innerchild *InnerContainer) AddMultipleAnswer(survey *Survey, text string, show_hints bool, layout SurveyLayout, data_question_visibility Visibility, has_other bool, mandatory JsonBool) *InnerContainer {
	...
	if innerchild.Class == ScaleRow {
		id, label := CreateID()
		survey.SurveyQNO++
		multianswer := &InnerContainer{Class: MultipleAnswer, Text: text, QNo: strconv.Itoa(survey.SurveyQNO) + "." + strconv.Itoa(len(innerchild.Children)+1) + ".a", Layout: layout.String(), HasOther: &has_other, ShowHints: &show_hints, Label: label, ID: id, Mandatory: mandatory.JBool()}
...
func (innerchild *InnerContainer) AddMultipleChoice(survey *Survey, text string, show_hints bool, layout SurveyLayout, data_question_visibility Visibility, has_other bool, mandatory JsonBool) *InnerContainer {
	...
	if innerchild.Class == ScaleRow ||  innerchild.Class == GridRow{
		id, label := CreateID()
		multianswer := &InnerContainer{Class: MultipleChoice, Text: text, QNo: strconv.Itoa(survey.SurveyQNO) + "." + strconv.Itoa(len(innerchild.Children)+1) + ".a", Layout: layout.String(), HasOther: &has_other, ShowHints: &show_hints, Label: label, ID: id, Mandatory: mandatory.JBool()}
		innerchild.Children = append(innerchild.Children, multianswer)
		return multianswer
	}
	return nil
